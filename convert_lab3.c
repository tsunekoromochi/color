...
#define HEIGHT        160
#define WIDTH         120
#define HEIGHT_START  130
#define WIDTH_START   30
#define HEIGHT_END    140
#define WIDTH_END     80
#define HEIGHT_START2 75
#define WIDTH_START2  50
#define HEIGHT_END2   100
#define WIDTH_END2    80
...
void convert_lab(double lab[][WIDTH][3], double lab2[][WIDTH][3], double lab3[][WIDTH][3])
{
  ...
  for(m=HEIGHT_START;m<HEIGHT_END+1;m++){
    for(n=WIDTH_START;n<WIDTH_END+1;n++){
      sum_l+=lab[m][n][0];
      sum_a+=lab[m][n][1];
      sum_b+=lab[m][n][2];
    }
  ...
  for(m=HEIGHT_START2;m<HEIGHT_END2+1;m++){
    for(n=WIDTH_START2;n<WIDTH_END2+1;n++){
      sum2_l+=lab2[m][n][0];
      sum2_a+=lab2[m][n][1];
      sum2_b+=lab2[m][n][2];
    }
  ...
  for(m=HEIGHT_START2;m<HEIGHT_END2+1;m++){
    for(n=WIDTH_START2;n<WIDTH_END2+1;n++){
      sum2_l+=pow(lab[m][n][0]-ave2_l,2);
      sum2_a+=pow(lab[m][n][1]-ave2_a,2);
      sum2_b+=pow(lab[m][n][2]-ave2_b,2);
    }
  }
  ...
  for(m=0;m<HEIGHT;m++){
    for(n=0;n<WIDTH;n++){
      ...
      lab[m][n][1]-=ave_a;
      lab[m][n][2]-=ave_b;
      //lab[m][n][0]=(hensa_l/hensa2_l)*lab[m][n][0];
      //printf("%lf %lf ",hensa_a,hensa2_a);
      //lab[m][n][1]=(hensa_a/hensa2_a)*lab[m][n][1];
      printf("%lf ",lab[m][n][2]);
      //lab[m][n][2]=(hensa_b/hensa2_b)*lab[m][n][2];
      lab[m][n][0]+=ave2_l;
      if(lab[m][n][0]<0){
	lab[m][n][0]=0;
      }
